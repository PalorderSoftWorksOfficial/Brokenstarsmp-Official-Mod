...
public class AnvilScreenHandler extends ForgingScreenHandler {
	...
	private static ForgingSlotsManager getForgingSlotsManager() {
		return ForgingSlotsManager.builder().input(0, 27, 47, itemStack -> true).input(1, 76, 47, itemStack -> true).output(2, 134, 47).build();
	}
	...
	public boolean setNewItemName(String string) {
		String string2 = sanitize(string);
		if (string2 != null && !string2.equals(this.newItemName)) {
			...
			if (this.getSlot(2).hasStack()) {
				...
				if (StringHelper.isBlank(string2)) {
					...
					itemStack.set(DataComponentTypes.CUSTOM_NAME, Text.literal(string2));
				}
			}
