...
public interface ScreenHandlerContext {
	 ...
	 */
	static ScreenHandlerContext create(World world, BlockPos blockPos) {
	 ...
	 * @param getter a function that gets a non-null value from this context's world and position
	 */
	default <T> T get(BiFunction<World, BlockPos, T> biFunction, T object) {
		return (T)this.get(biFunction).orElse(object);
	}
