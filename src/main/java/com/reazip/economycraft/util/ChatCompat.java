...
public final class ChatCompat {
    ...
    private static volatile Constructor<?> enumCtorComponent;     // ClickEvent(Action, Text)
    private static volatile Constructor<?> nestedCtorString;      // ClickEvent$RunCommand(String) or similar
    private static volatile Constructor<?> nestedCtorComponent;   // ClickEvent$RunCommand(Text) or similar
    ...
    public static ClickEvent runCommandEvent(String cmd) {
        ...
        try {
            if (runCommandEnum != null) {
                ...
                if (enumCtorComponent != null) {
                    Object ev = enumCtorComponent.newInstance(runCommandEnum, Text.literal(cmd));
                    if (ev instanceof ClickEvent ce && isRunCommand(ce)) {
                        return ce;
                    }
                }
        ...
        try {
            ...
            if (nestedCtorComponent != null) {
                Object ev = nestedCtorComponent.newInstance(Text.literal(cmd));
                if (ev instanceof ClickEvent ce && isRunCommand(ce)) {
                    return ce;
                }
            }
     ...
     */
    public static void sendRunCommandTellraw(ServerPlayerEntity target, String prefixText, String labelText, String cmd) {
        try {
            ...
            String selector = target.getGameProfile().name(); // safe for tellraw target
            String line = "tellraw " + selector + " " + json;

            var srv = target.getEntityWorld().getServer();
            srv.getCommandManager().parseAndExecute(
                    PermissionCompat.withOwnerPermission(srv.getCommandSource()),
                    line);
        } catch (Throwable ignored) {
            ...
            try {
                ...
                for (Constructor<?> c : ClickEvent.class.getDeclaredConstructors()) {
                    ...
                    if (p.length == 2 && actionEnumType != null && p[0].isAssignableFrom(actionEnumType)) {
                        ...
                        else if (Text.class.isAssignableFrom(p[1])) enumCtorComponent = c;
                    }
                }

                // C) Find nested ClickEvent subclasses that accept (String) / (Text)
                for (Class<?> nested : ClickEvent.class.getDeclaredClasses()) {
                    ...
                    try {
                        Constructor<?> cCtor = nested.getConstructor(Text.class);
                        Object probe = cCtor.newInstance(Text.literal("/ec probe"));
                        if (probe instanceof ClickEvent ce && isRunCommand(ce)) {
